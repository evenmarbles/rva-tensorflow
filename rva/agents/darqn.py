...
# TODO: currently this is just a copy of DRQN
class DARQNSoftAgent(BaseModel):
    @property
    def name(self):
        return 'DARQN'
    ...
    def __init__(self, args, env, min_reward=-1.0, max_reward=1.0, sess=None):
        ...
        super(DARQNSoftAgent, self).__init__(args, env, min_reward, max_reward, sess, True)
    ...
    def _build(self, args):
        ...
        initial_prediction = tf.Variable(0, dtype='float32', trainable=False, validate_shape=False)
        ...
        with tf.variable_scope('prediction') as vs:
            ...
            def body(i, z_prev, lstm_c, lstm_h):
                _in = inpt[i]
                ...
                z_prev = tf.cond(tf.equal(i, 0),
                                 lambda: tf.assign(initial_prediction, tf.expand_dims(q_pred, 0), validate_shape=False),
                                 lambda: tf.concat(0, [z_prev, tf.expand_dims(q_pred, 0)]))
                i_next = tf.add(i, 1)
                return i_next, z_prev, _state.c, _state.h

            _, self.q, self.next_state_c, self.next_state_h = tf.while_loop(
                lambda i, pred, lstm_c, lstm_h: tf.less(i, history_length), body,
                [tf.constant(0), initial_prediction, self.lstm_c, self.lstm_h])
        ...
        with tf.variable_scope('optimizer'):
            ...
            def optimizer_body(i, z_prev):
                ...
                z_prev = tf.cond(tf.equal(i, 0),
                                 lambda: tf.assign(initial_prediction, tf.expand_dims(clipped_delta, 0),
            ...
            _, self.clipped_delta = tf.while_loop(
                lambda i, delta: tf.less(i, history_length), optimizer_body,
                [tf.constant(0), initial_prediction])
